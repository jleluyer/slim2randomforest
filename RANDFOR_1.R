...
matrix.geno<-read.table("03_matrices/matrix_genetic.1.txt",header=TRUE)
names.matrix<-read.table("03_matrices/loci.matrix.1.txt",header=F)
rownames(matrix.geno) = names.matrix [,1]

#upload admixture
matrix.admixture<-read.table("03_matrices/matrix.admixture.1.txt",header=F)
...
#abline(h=25)
save(rf3,file="04_results/rf3.1.Rda")
#dev.off()
message("rf ok")
#pdf("plotgrp3.pdf")
...
write.table(marker.names40,file="04_results/selected_markers40.1.txt",quote=F)
...
write.table(marker.names25,file="04_results/selected_markers25.1.txt",quote=F)
